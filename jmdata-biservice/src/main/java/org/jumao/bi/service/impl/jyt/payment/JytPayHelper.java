...
public class JytPayHelper {
    ...
    public static TopLineChart getTopXY(Map<String, String> map, TopLineChart topLineChart, String flag) {
        ...
        for (Map.Entry<String, String> entry : map.entrySet()) {  //  循环数据库结果，value值给x轴，key名称给y轴
            ...
            if (flag.equals("baseareaMap")) {  //  根据code获取name
                seriesData[i] = baseareaMap.get(entry.getKey());  //  省名称不需要脱敏
            } else if (flag.equals("lgcompanyMap")) {
                seriesData[i] = DesensitizationUtils.getDesStr(lgcompanyMap.get(entry.getKey()));
            }
            i++;
        }
...
//            hashMap.put(entry.getKey(), entry.getValue());
    ...
    public static Map<String, String> getProvinceName(Map<String, String> map, String flag) {
        ...
        for (Map.Entry<String, String> entry : map.entrySet()) {
            String keyName = "";
            if (flag.equals("baseareaMap")) {  //  地图list，name/value
                keyName = baseareaMap.get(entry.getKey());
            } else if (flag.equals("baseDataOptionMap")) {  //  车辆分析，饼图list，value/name
                keyName = baseDataOptionMap.get(entry.getKey());
    ...
    public static Map<String, String> washData(Map<String, String> map) {
        ...
        while (iter.hasNext()) {  //  去掉脏数据
            ...
            if (!(key.length() == 5) || key == "null" || key.equals("null")) {
                iter.remove();
            }
        }
        return map;
...
//            hashMap.put(entry.getValue(),entry.getKey());
    ...
    public static List<List<Map<String, String>>> plaMap(Map<String, String> map, String flag){
        ...
        for (Map.Entry<String, String> entry : map.entrySet()) {
            ...
            Map<String, String> hashMap = new LinkedHashMap<String, String>();
            if (flag.equals("airportsMap")){
                String name1 = airportsMap.get(key1);
                String name2 = airportsMap.get(key2);
                if (name1 == null || name2 == null) continue;
                nameMap.put("name", name1);
                hashMap.put("name", name2);
            }else if (flag.equals("baseareaMap")){
                String name1 = baseareaMap.get(key1);
                String name2 = baseareaMap.get(key2);
                if (name1 == null || name2 == null) continue;
                name1 = subName(name1);
                name2 = subName(name2);
                nameMap.put("name", name1);
                hashMap.put("name", name2);
            }else if (flag.equals("innerportsMap")){
                String name1 = innerportsMap.get(key1);
                String name2 = innerportsMap.get(key2);
                if (name1 == null || name2 == null) continue;
                nameMap.put("name", name1);
                hashMap.put("name", name2);
            }else if (flag.equals("lgportsMap")){
                String name1 = lgportsMap.get(key1);
                String name2 = lgportsMap.get(key2);
                if (name1 == null || name2 == null) continue;
    ...
    public static List<List<Map<String, String>>> plaMap_(Map<String, String> map, String flag){
        ...
        for (Map.Entry<String, String> entry : map.entrySet()) {
            ...
            Map<String, String> hashMap = new LinkedHashMap<String, String>();
            if (flag.equals("baseareaMap")){
                String name1 = baseareaMap.get(key);
                String name2 = baseareaMap.get(key);
                if (name1 == null || name2 == null) continue;
                name1 = subName(name1);
                name2 = subName(name2);
                nameMap.put("name", name1);
