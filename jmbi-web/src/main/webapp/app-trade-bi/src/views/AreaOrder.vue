  ...
  <div>
    ...
    <section class="order-content jm-grid-box-bg-white jm-grid-border-1px jm-grid-box-border-radius">
      ...
      <aside class="order-data-summary">
        <header>
          <ul class="clearfix">
            <li class="left" v-bind:class="{ 'is-active': currentSelectedPeriod === 'week' }" @click="selectPeriod('week')">
              本周
            </li>
            <li class="left" v-bind:class="{ 'is-active': currentSelectedPeriod === 'month' }" @click="selectPeriod('month')">
              本月
            </li>
            <li class="left " v-bind:class="{ 'is-active': currentSelectedPeriod === 'quarter' }" @click="selectPeriod('quarter')">
              本季
            </li>
            <li class="left " v-bind:class="{ 'is-active': currentSelectedPeriod === 'year' }" @click="selectPeriod('year')">
              本年
            </li>
          </ul>
        </header>
        <section class="order-data-summary-data-list" v-loading="loadingTopList" element-loading-text="加载中...">
              ...
              <table class="jm-table">
                <thead>
                  <tr>
                    ...
                    <th class="jm-text-align-right number-head-col">
                      <span>订单量（票）</span>
                    </th>
                  </tr>
                </thead>
                <tbody>
                  <tr v-for="(item, index) in areaOrderList.c">
                    ...
                    <td class="jm-text-align-right" :title="item.value | numberWithCommas">{{item.value | numberWithCommas}}</td>
                  </tr>
                </tbody>
              </table>
            </div>
    ...
    <!--dialog start -->
    <!--订单货值趋势-->
    <order-details :popupVisible="dialogOrderPriceChartVisible" v-on:closeMe="dialogOrderPriceChartVisible = false" :orderType="2" :dialogTitle="dialogOrderPriceTitle">
    </order-details>
  
    <!--订单数趋势-->
    <order-details :popupVisible="dialogOrderAmountChartVisible" v-on:closeMe="dialogOrderAmountChartVisible = false" :orderType="1" :dialogTitle="dialogOrderAmountTitle">
    </order-details>
    <!--dialog end-->
  ...
  methods: {
    loadorderChinaMapChart() {
      ...
      function loadMapData(chartData) {
        ...
        vm.orderChinaMapChart.off('click')
